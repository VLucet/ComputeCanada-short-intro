...
A very good resource we will come back to many times today:
...
* Use GLOBUS. It is a browser interface with the clusters' file systems, allowing you to transfer files and sync whole directories. See [this wiki page]() for setting it up.
...
1. Send jobs to the SLURM job manager with `sbatch`. 

2. Start an interactive session within the terminal with `salloc`
